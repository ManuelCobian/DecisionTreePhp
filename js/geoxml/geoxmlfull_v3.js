...
//  c.style.fontSize = "0px";
  ...
  this.publishdirectory = base_url+"/img/cart/";
  topwin = top;
  ...
  // Is this the last file to be processed?
	...
	if(update_side && count>0){
		 if (overlaymanager.paren.progress == 0) {
			...
			overlaymanager.paren.mb.showMess("Se cargaron "+count+"  elementos en "+sec,5000);
...
GeoXml.prototype.DownloadURL = function (fpath,callback,title, xmlcheck){
    ...
    cmlreq.onreadystatechange = function () {
        switch (cmlreq.readyState) {
            case 4:
                that.mb.showMess(title+" recibido", 2000);
                if (typeof ActiveXObject != "undefined") {
		    ...
		    callback(response);
                    ...
                    if (cmlreq.responseXML) {
					  ...
					  if (cmlreq.status == 200) {
							...
							if (!xmlcheck||(isXML == "<?xml" && sresp.indexOf("kml")!=-1)) {
								that.mb.showMess(title+" respuesta recibida", 2000);
								callback(resp);
								}
							else {
                ...
                break;
            case 3:
                that.mb.showMess("Recibiendo "+title+"...",2000);
                break;
            case 2:
                that.mb.showMess("Esperando por "+title,2000);
                break;
            case 1:
                that.mb.showMess("Peticion enviada para "+title,2000);
                break;
            }
        };
